...
[Sample(SampleOrder.CloudUpload, 
    "Cloud upload",
    "The sample illustrates how to implement Cloud (Azure ADLS) upload to a RelativityOne file share. ", 
    typeof(CloudUpload), 
    TransferType.CloudUpload)]
internal class CloudUpload : ISample
{
    ...
    public CloudUpload(IConsoleLogger consoleLogger, IPathExtension pathExtension, IRelativityAuthenticationProviderFactory relativityAuthenticationProviderFactory, IProgressHandlerFactory progressHandlerFactory)
    {
    ...
    public async Task ExecuteAsync(Configuration configuration, CancellationToken token)
    {
        ...
        var source = new CloudLocation(configuration.CloudUpload.Source);
        var destination = string.IsNullOrWhiteSpace(configuration.CloudUpload.Destination)
            ? _pathExtension.GetDefaultRemoteDirectoryPathForUpload(configuration.Common)
            : new DirectoryPath(configuration.CloudUpload.Destination);
        ...
        var result = await transferClient
            .CloudUploadAsync(jobId, source, destination, new CloudUploadOptions(), progressHandler, token)
